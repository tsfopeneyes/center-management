...
async function run() {
    ...
    const { data: logs } = await supabase.from('logs').select('*').gte('created_at', start.toISOString()).lte('created_at', end.toISOString());

    const sessions = aggregateVisitSessions(logs, users, locations, format(start, 'yyyy-MM-dd'), format(end, 'yyyy-MM-dd'));
    const totalDurationByApp = new Map();
    ...
    sessions.forEach(session => {
        ...
        totalDurationByApp.set(userId, (totalDurationByApp.get(userId) || 0) + sessionTotal);
    });

    console.log('--- Individual User Durations (Emulated) ---');
    totalDurationByApp.forEach((dur, uid) => {
        const user = users.find(u => u.id === uid);
        console.log(`User: ${user.name} | Duration: ${Math.floor(dur / 60)}h ${dur % 60}m`);
    });

    let centerTotal = 0;
    totalDurationByApp.forEach(v => centerTotal += v);
    console.log(`\nGrand Total: ${Math.floor(centerTotal / 60)}h ${centerTotal % 60}m`);
}
run();
