...
const aggregateVisitSessions = (allLogs, users, locations, startDate, endDate) => {
    ...
    visitLogs.forEach(log => {
        ...
        if (!userDateGroups[groupKey]) {
            ...
            userDateGroups[groupKey] = { userId: log.user_id, rawLogs: [] };
        }
    ...
    Object.values(userDateGroups).forEach(group => {
        ...
        sessions.forEach(sessionLogs => aggregated.push({ userId: group.userId, rawLogs: sessionLogs }));
    });
    return aggregated;
};
...
async function run() {
    ...
    // 1. Method A: ProcessAnalyticsData (Sessions) - EXISTING LOGIC
    ...
    sessions.forEach(session => {
        ...
        durationLogs.forEach((log) => {
            if (log.type === 'MOVE') {
                statsTotal += differenceInMinutes(new Date(log.created_at), segmentStart);
                currentLocId = log.location_id;
                segmentStart = new Date(log.created_at);
            }
        });
        if (durationLogs[durationLogs.length - 1].type === 'CHECKOUT') {
            statsTotal += differenceInMinutes(new Date(durationLogs[durationLogs.length - 1].created_at), segmentStart);
        }
    });

    // 2. Method B: ProcessOperationReport (Direct Loop) - FIXED LOGIC
    let reportTotal = 0;
    ...
    filteredLogs.forEach(log => {
        ...
        if (current) reportTotal += differenceInMinutes(logTime, current.time);
        ...
        if (log.type === 'CHECKIN' || log.type === 'MOVE') {
            ...
            // THE FIX:
            userCurrent.set(userId, { ...current, time: logTime });
        }
    });

    console.log(`Stats Total: ${Math.floor(statsTotal / 60)}h ${statsTotal % 60}m`);
    console.log(`Report Total: ${Math.floor(reportTotal / 60)}h ${reportTotal % 60}m`);
}
run();
