-- Keep the unique index as the final concurrency guard, but reject duplicate
-- fixed-date/daily completion with a user-facing message before it reaches the
-- index error.
...
BEGIN
    ...
    IF v_mission.schedule_type = 'FIXED_DATE' THEN
        ...
        IF EXISTS (
            SELECT 1 FROM public.online_challenge_submissions s
            WHERE s.mission_id = v_mission.id
              AND s.participant_id = NEW.participant_id
              AND s.completion_key = NEW.completion_key
              AND s.is_valid
        ) THEN
            RAISE EXCEPTION '이미 완료한 미션입니다.' USING ERRCODE = '23514';
        END IF;
    ELSIF v_mission.schedule_type = 'DAILY' THEN
        ...
        IF EXISTS (
            ...
            RAISE EXCEPTION '오늘은 이미 이 미션을 완료했습니다.' USING ERRCODE = '23514';
        END IF;
    ELSE
