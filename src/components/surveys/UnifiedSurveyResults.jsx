...
function summarize(entries) {
    ...
    entries.filter(entry => !entry.aggregation_excluded && !entry.legacy).forEach(entry => {
...
export default function UnifiedSurveyResults({ form, entries, formLinks, label, filter, setFilter, versionFilter, setVersionFilter, showExcluded, setShowExcluded, busy, onBack, onToggleExclude }) {
    ...
    const countFor = id => id === 'ALL' ? entries.length : entries.filter(entry => entry.link_id === id).length;
    return <div className="space-y-6">
        ...
        <div className="space-y-6">{groups.map(group => <ResultCard key={group.key} group={group} />)}{!groups.length && included.length > 0 && <p className="rounded-[24px] border border-gray-100 bg-white p-8 text-center text-sm text-gray-400 shadow-sm">비교 가능한 새 형식 응답이 없습니다. 과거 응답은 아래 표에서 확인할 수 있습니다.</p>}</div>
