...
const LogCard = ({
    ...
    return (
        <div ref={logRef} className="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden scroll-mt-24">
            {/* Header / Meta Info */}
            <div className="p-4 md:p-8 border-b border-gray-50 bg-gray-50/30 flex flex-col md:flex-row justify-between items-start gap-4">
                <div className="space-y-3 flex-1 w-full min-w-0">
                    <div className="flex flex-wrap items-center gap-2 md:gap-3">
                        ...
                        <div className="px-3 py-1 bg-indigo-100/50 rounded-lg text-xs font-black text-indigo-600 shrink-0">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 md:gap-4">
                        ...
                        {isEditing ? (
                            <input
                                ...
                                className="w-full sm:w-auto sm:flex-1 max-w-[200px] px-3 py-1.5 bg-white border border-indigo-200 rounded-lg text-xs font-bold text-gray-700 outline-none focus:border-indigo-500 shadow-sm"
                            />
                        ) : (
                ...
                <div className="flex gap-2 shrink-0 self-end md:self-start w-full md:w-auto justify-end">
                    {isEditing ? (
                        <button
                            onClick={() => handleSaveEdit(log.id)}
                            disabled={savingId === log.id}
                            className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 md:px-4 md:py-2 bg-indigo-600 text-white rounded-xl text-sm md:text-xs font-black shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95 disabled:opacity-50 whitespace-nowrap"
                        >
                            <Save size={16} className="md:w-3.5 md:h-3.5" />
                            저장
                        </button>
                    ) : (
                        <button
                ...
                <div className="space-y-3">
                    ...
                    {isEditing ? (
                        <div className="flex flex-wrap gap-2 relative border border-gray-100 p-3 rounded-2xl bg-gray-50/50">
                            {(staffList || []).filter(s => Array.isArray(school.metadata?.manager_ids) && school.metadata.manager_ids.includes(s.id)).map(staff => (
                                <button
                                    key={staff.id}
                                    onClick={() => toggleFacilitator(log.id, staff.id)}
                                    className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-[11px] font-bold transition-all border ${Array.isArray(editData.facilitator_ids) && editData.facilitator_ids.includes(staff.id) ? 'bg-indigo-600 text-white border-indigo-600 shadow-md' : 'bg-white text-gray-500 border-indigo-100 hover:bg-indigo-50'}`}
                                >
                                    <span>{staff.name}</span>
                                </button>
                            ))}
                            {(!staffList?.some(s => Array.isArray(school.metadata?.manager_ids) && school.metadata.manager_ids.includes(s.id))) && (
                        ...
                        <div className="grid grid-cols-3 sm:grid-cols-4 gap-1.5 md:gap-2 max-h-[140px] md:max-h-[160px] overflow-y-auto no-scrollbar border border-gray-100 rounded-2xl bg-gray-50/50 p-3">
                            {filteredStudents.map(student => (
                                <button
                                    ...
                                    className={`flex items-center justify-center p-1.5 md:p-2 rounded-xl text-[10px] font-black transition-all border ${Array.isArray(editData.participant_ids) && editData.participant_ids.includes(student.id) ? 'bg-indigo-600 text-white border-indigo-600 shadow-md' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-50'}`}
                                >
