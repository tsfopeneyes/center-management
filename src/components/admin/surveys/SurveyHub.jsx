...
import { ArrowLeft, Check, ClipboardList, Copy, Eye, Plus, Search } from 'lucide-react';
...
export default function SurveyHub({ onLegacy, notices = [], responses = [], visitNotes = [], users = [] }) {
    ...
    const [query, setQuery] = useState(''), [applyVersion, setApplyVersion] = useState(null), [applyIds, setApplyIds] = useState([]);
    ...
    const reload = async () => {
        ...
        try {
            ...
            const modernEntryCounts = countBy(allEntries, 'form_id');
    ...
    const label = link => link.id === 'LEGACY' || link.link_id === 'LEGACY' ? '이전 설문 응답' : link.notice_id ? (programs.find(p => String(p.id) === String(link.notice_id))?.title || '프로그램') : `${SURVEY_CENTERS.find(c => c.code === link.center_code)?.label} ${link.event === 'CHECKIN' ? '입실' : '퇴실'}`;
    const loadResultEntries = async id => {
        return surveyHubApi.entries(id);
    };
    ...
    const setVisitConnection = async (targetForm, centerCode, event) => {
        ...
        if (activeForThisForm) {
            await surveyHubApi.updateLink(activeForThisForm.id, { enabled: false });
            await reload();
            return;
        }
        for (const conflict of links.filter(link => link.enabled && link.center_code === centerCode && link.event === event)) {
            await surveyHubApi.updateLink(conflict.id, { enabled: false });
        }
        const reusable = links.find(link => !link.enabled && link.form_id === targetForm.id && link.center_code === centerCode && link.event === event);
        if (reusable) await surveyHubApi.updateLink(reusable.id, { enabled: true, version_id: latest(targetForm).id, frequency: 'EVERY_VISIT', priority: 100, is_default: false, opens_at: null, closes_at: null });
        else await surveyHubApi.connect({ form_id: targetForm.id, version_id: latest(targetForm).id, event, center_code: centerCode, notice_id: null, frequency: 'EVERY_VISIT', priority: 100, is_default: false });
        await reload();
    };
    return <div className="space-y-6 pb-12 animate-fade-in-up">
        ...
        {ready && !loading && <>
            {view === 'import' && <button onClick={() => { setView('list'); setEditor(null); setApplyVersion(null); }} className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-bold hover:bg-gray-50"><ArrowLeft size={17} />설문 목록</button>}
            {view === 'list' && <>
                ...
                {sectionTab === 'SURVEYS' && applyVersion && <section className="space-y-4 rounded-[24px] border border-blue-200 bg-blue-50 p-5 shadow-sm"><div><h3 className="font-bold text-gray-900">바꾼 질문을 어디에 적용할까요?</h3><p className="mt-1 text-sm text-gray-600">선택한 곳만 새 질문으로 바뀌고, 이미 받은 응답은 그대로 보존됩니다.</p></div><div className="grid gap-2 md:grid-cols-2">{formLinks.filter(link => link.enabled && link.version_id !== applyVersion.id).map(link => <label key={link.id} className="flex cursor-pointer items-center gap-3 rounded-xl border border-blue-100 bg-white p-3 text-sm font-bold"><input type="checkbox" className="h-4 w-4 rounded border-gray-300 text-blue-600" checked={applyIds.includes(link.id)} onChange={event => setApplyIds(ids => event.target.checked ? [...ids, link.id] : ids.filter(id => id !== link.id))} />{label(link)}</label>)}</div><div className="flex flex-wrap gap-2"><button className="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-40" disabled={busy || !applyIds.length} onClick={() => execute(async () => { for (const id of applyIds) await surveyHubApi.updateLink(id, { version_id: applyVersion.id }); await reload(); setApplyVersion(null); setApplyIds([]); setSelected(null); })}>선택한 곳에 적용</button><button className="rounded-xl px-4 py-2.5 text-sm font-bold text-gray-500 hover:bg-white/60" onClick={() => { setApplyVersion(null); setSelected(null); }}>닫기</button></div></section>}
                {sectionTab === 'SURVEYS' && <div className="overflow-hidden rounded-[24px] border border-[#f2f4f6] bg-white shadow-sm"><div className="hidden grid-cols-[minmax(220px,.8fr)_80px_70px_minmax(410px,1.5fr)_210px] gap-3 bg-gray-50 px-5 py-3 text-xs font-bold text-gray-500 md:grid"><span>설문</span><span>구분</span><span>응답</span><span>입실·퇴실 설정</span><span>관리</span></div>{visibleForms.map(f => { const activeLinks = links.filter(l => l.form_id === f.id && l.enabled && l.event !== 'PROGRAM'); const types = f.kind === 'TEMPLATE' ? ['TEMPLATE'] : [...new Set(activeLinks.map(l => l.event))]; return <div key={f.id} role="button" tabIndex={0} onClick={() => execute(() => loadResults(f.id))} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); execute(() => loadResults(f.id)); } }} className="grid cursor-pointer items-center gap-3 border-t border-gray-100 px-5 py-4 transition-colors hover:bg-blue-50/40 focus:bg-blue-50 focus:outline-none md:grid-cols-[minmax(220px,.8fr)_80px_70px_minmax(410px,1.5fr)_210px]"><div><p className="font-bold text-gray-900">{latest(f)?.definition.title || f.title}{f.archived && <span className="ml-2 text-xs text-gray-400">보관됨</span>}</p><p className="mt-1 text-xs text-gray-400">{latest(f)?.definition.questions.length || 0}개 질문</p></div><div className="flex flex-wrap gap-1">{types.length ? types.map(type => <span key={type} className={`w-fit px-2 py-1 text-xs font-bold ${type === 'CHECKOUT' ? 'bg-emerald-50 text-emerald-700' : type === 'TEMPLATE' ? 'bg-violet-50 text-violet-700' : 'bg-blue-50 text-blue-700'}`}>{type === 'CHECKIN' ? '입실' : type === 'CHECKOUT' ? '퇴실' : '템플릿'}</span>) : <span className="text-xs text-gray-400">미연결</span>}</div><span className="text-sm font-bold text-blue-600">{formEntryCounts[f.id] || 0}건</span><div onClick={event => event.stopPropagation()}><div className="grid grid-cols-2 gap-1.5">{SURVEY_CENTERS.flatMap(center => ['CHECKIN','CHECKOUT'].map(surveyEvent => { const active = links.find(link => link.enabled && link.center_code === center.code && link.event === surveyEvent); const isActive = active?.form_id === f.id; return <button key={`${center.code}-${surveyEvent}`} type="button" disabled={busy || f.archived || f.kind === 'TEMPLATE'} onClick={() => execute(() => setVisitConnection(f, center.code, surveyEvent))} className={`rounded-lg border px-2 py-2 text-xs font-bold transition ${isActive ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 bg-white text-gray-400 hover:border-blue-300 hover:text-blue-600'} disabled:bg-gray-50 disabled:text-gray-300`}>{isActive && <Check className="mr-1 inline" size={13} />}{center.label} {surveyEvent === 'CHECKIN' ? '입실' : '퇴실'}</button>; }))}</div></div><div className="flex flex-wrap gap-2"><button className="rounded-xl border border-gray-200 bg-white px-3 py-2 text-xs font-bold hover:bg-gray-50" onClick={event => { event.stopPropagation(); setSelected(f.id); setEditor(latest(f)?.definition || {}); setView('edit'); }}>편집</button><button title="복제" className="rounded-xl border border-gray-200 bg-white p-2 hover:bg-gray-50" onClick={event => { event.stopPropagation(); setSelected(null); setEditor({ ...latest(f)?.definition, legacySource: null, title: `${latest(f)?.definition.title || f.title} 복사본` }); setView('edit'); }}><Copy size={15} /></button><button className="rounded-xl border border-gray-200 bg-white px-3 py-2 text-xs font-bold hover:bg-gray-50" disabled={busy} onClick={event => { event.stopPropagation(); execute(async () => { await surveyHubApi.archive(f.id, !f.archived); await reload(); }); }}>{f.archived ? '보관 해제' : '보관'}</button></div></div>; })}{!visibleForms.length && <p className="p-10 text-center text-sm text-gray-400">새 설문을 만들거나 기존 설문을 가져와 시작하세요.</p>}</div>}
            ...
            {view === 'edit' && <SurveyDefinitionEditor key={`${selected || 'new'}-${view}`} initial={editor?.questions ? editor : undefined} allowTemplate={!selected} busy={busy} onCancel={() => setView('list')} onSave={async (definition, options) => {
                ...
                try {
                    ...
                    if (editedForm?.kind === 'PROGRAM') {
                        const programLink = links.find(link => link.enabled && link.form_id === editedForm.id && link.event === 'PROGRAM');
                        if (!programLink) throw new Error('이 프로그램의 설문 연결을 찾을 수 없습니다.');
                        await surveyHubApi.updateLink(programLink.id, { version_id: version.id });
                        await reload();
                        setSelected(null); setEditor(null); setSectionTab('PROGRAMS'); setView('list');
                        return;
                    }
                    const hasConnections = links.some(link => link.enabled && link.form_id === version.form_id);
                    await reload(); setSelected(hasConnections ? version.form_id : null); setApplyVersion(hasConnections ? version : null); setApplyIds([]); setView('list');
                }
                finally { setBusy(false); }
            }} />}
            {view === 'results' && form && <UnifiedSurveyResults form={form} entries={entries} formLinks={formLinks} label={label} filter={filter} setFilter={setFilter} versionFilter={versionFilter} setVersionFilter={setVersionFilter} showExcluded={showExcluded} setShowExcluded={setShowExcluded} busy={busy} onBack={() => { setView('list'); setSelected(null); }} onToggleExclude={entry => execute(async () => { const result = await supabase.from('survey_entries').update({ aggregation_excluded: !entry.aggregation_excluded }).eq('id', entry.id); if (result.error) throw result.error; setEntries(await surveyHubApi.entries(selected)); })} />}
        </>}
