...
export default function SurveyHub({ onLegacy, notices = [], responses = [], visitNotes = [], users = [] }) {
    ...
    const selectedPublicLinks = selected ? links.filter(link => link.form_id === selected && link.event === 'PUBLIC') : [];
    const selectedPublicLink = selectedPublicLinks.find(link => link.enabled) || selectedPublicLinks.at(-1) || null;
    ...
    const copyPublicLink = async link => {
        const url = `${window.location.origin}/survey/${link.public_slug || link.public_token}`;
        await navigator.clipboard.writeText(url);
        window.alert('설문 링크를 복사했습니다.');
    };
    const deleteForm = async target => {
        const title = latest(target)?.definition.title || target.title;
        const source = [...(target.survey_versions || [])].sort((a, b) => a.created_at.localeCompare(b.created_at))[0]?.definition?.legacySource;
        const legacyNote = source?.table === 'surveys' ? '\n이전 출입 기록 중 설문 ID가 없는 원본 기록은 유지됩니다.'
            : source?.table === 'program_feedback' ? '\n프로그램의 원본 피드백 기록은 유지됩니다.' : '';
        if (!window.confirm(`‘${title}’ 설문을 영구 삭제할까요?\n설문, 공유·입실·퇴실 연결과 이 설문에 저장된 응답이 삭제되며 되돌릴 수 없습니다.${legacyNote}`)) return;
        await execute(async () => {
            await surveyHubApi.deleteForm(target.id);
            await reload();
        });
    };
    return <div className="space-y-6 pb-12 animate-fade-in-up">
        ...
        {ready && !loading && <>
            ...
            {view === 'edit' && <SurveyDefinitionEditor key={`${selected || 'new'}-${view}`} initial={editor?.questions ? editor : undefined} allowTemplate={!selected} allowPublicAddress={!selected || form?.kind === 'SURVEY'} initialPublicSlug={selectedPublicLink?.public_slug || ''} requirePublicSlug={!selected} busy={busy} onCancel={() => setView('list')} onSave={async (definition, options) => {
                setBusy(true);
                try {
                    const editedForm = selected ? forms.find(item => item.id === selected) : null;
                    const version = selected ? await surveyHubApi.publish(selected, definition) : await surveyHubApi.create(definition, options?.saveAsTemplate ? 'TEMPLATE' : 'SURVEY', options?.publicSlug);
                    if (editedForm) {
                        ...
                        if (editedForm.kind === 'SURVEY' && options?.publicSlug && options.publicSlug !== selectedPublicLink?.public_slug) {
                            if (selectedPublicLink) await surveyHubApi.replacePublicLink(editedForm, selectedPublicLink, options.publicSlug, version);
                            else await surveyHubApi.createPublicLink({ ...editedForm, survey_versions: [version] }, options.publicSlug);
                        }
