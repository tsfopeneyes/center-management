...
import { Calendar, Clock, MapPin, Gift, CheckSquare, Users, Plus, Trash2, HelpCircle } from 'lucide-react';
...
const ProgramInfoSection = ({ formData, updateField, flat = false }) => {
    ...
    const handleAddMission = () => {
        const currentMissions = formData.challenge_missions || [];
        const newMission = {
        ...
        updateField('challenge_missions', [...currentMissions, newMission]);
    };

    const handleRemoveMission = (idx) => {
        const currentMissions = formData.challenge_missions || [];
        const nextMissions = currentMissions.filter((_, i) => i !== idx);
        updateField('challenge_missions', nextMissions);
    };

    const handleUpdateMission = (idx, field, value) => {
        const currentMissions = formData.challenge_missions || [];
        const nextMissions = currentMissions.map((m, i) => {
            if (i === idx) {
        ...
        <div className="space-y-6">
            ...
            <div className={flat 
                ...
                <div className={flat 
                    ...
                    <button
                        type="button"
                        onClick={() => {
                            updateField('program_type', PROGRAM_TYPES.CENTER);
                        }}
                        className={flat 
                    ...
                    <button
                        type="button"
                        onClick={() => {
                            updateField('program_type', PROGRAM_TYPES.SCHOOL_CHURCH);
                        }}
                        className={flat 
                            ? `flex-1 py-2 rounded-lg text-sm font-bold transition-all duration-200 ${
                    ...
                    <button
                        type="button"
                        onClick={() => {
                            updateField('program_type', PROGRAM_TYPES.CHALLENGE);
                            updateField('is_recruiting', false);
                            updateField('max_capacity', 0);
                            updateField('is_review_required', false);
                            if (!formData.hyphen_reward) {
                                updateField('hyphen_reward', 30);
                            }
                        }}
                        className={flat 
                            ? `flex-1 py-2 rounded-lg text-sm font-bold transition-all duration-200 ${
                                formData.program_type === PROGRAM_TYPES.CHALLENGE
                                    ? 'bg-white text-slate-900 shadow-sm font-extrabold'
                                    : 'text-slate-600 hover:text-slate-900'
                              }`
                            : `px-5 py-2 rounded-lg text-sm font-bold transition-all duration-200 ${
            ...
            {isChallenge ? (
                ...
                <div className="space-y-4">
                    <span className="text-xs font-semibold text-slate-500 ml-1">뱃지 상세 설정</span>
                    <div className="border border-slate-200/70 rounded-2xl bg-white overflow-hidden shadow-sm p-5 space-y-5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    onChange={e => updateField('challenge_start_date', e.target.value)}
                                    className="w-full p-3 bg-slate-50/50 border border-slate-200/80 rounded-xl outline-none font-semibold text-slate-700 text-sm focus:border-blue-600 focus:bg-white transition-all"
                                    required
                                />
                            </div>
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    onChange={e => updateField('challenge_end_date', e.target.value)}
                                    className="w-full p-3 bg-slate-50/50 border border-slate-200/80 rounded-xl outline-none font-semibold text-slate-700 text-sm focus:border-blue-600 focus:bg-white transition-all"
                                    required
                                />
                            </div>
                        ...
                        <div className="border-t border-slate-100 pt-4">
                            <div className="space-y-2 max-w-md">
                                ...
                                <input
                                    ...
                                    required
                                />
                                <p className="text-[10px] text-slate-400 ml-1">모든 미션을 완료한 경우, 학생 지갑으로 해당 수량만큼 H가 자동 지급됩니다.</p>
                            </div>
                        </div>
                        ...
                        <div className="border-t border-slate-100 pt-4 space-y-3">
                            <div className="flex items-center justify-between">
                                <span className="text-xs font-bold text-slate-600">뱃지 미션 구성 ({formData.challenge_missions?.length || 0}개)</span>
                                <button
                                    type="button"
                            ...
                            <div className="space-y-3">
                                {(!formData.challenge_missions || formData.challenge_missions.length === 0) ? (
                                    <div className="text-center py-8 border-2 border-dashed border-slate-100 rounded-2xl text-slate-400 text-xs font-medium">
                                        추가된 미션이 없습니다. [미션 추가] 버튼을 눌러 미션을 작성해 주세요.
                                    </div>
                                ) : (
                                    formData.challenge_missions.map((mission, idx) => (
                                        <div key={mission.id} className="p-4 bg-slate-50/60 border border-slate-200/60 rounded-2xl space-y-3 relative group">
                                            <button
                                                type="button"
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pr-8">
                                                ...
                                                <div className="space-y-1">
                                                    <label className="text-[10px] font-bold text-slate-500">미션명</label>
                                                    <input
                                                        type="text"
                                                        placeholder="예: 고요한 연결"
                                                        value={mission.title || ''}
                                                        onChange={e => handleUpdateMission(idx, 'title', e.target.value)}
                                                        className="w-full px-3 py-2 bg-white border border-slate-200/80 rounded-lg outline-none font-semibold text-slate-700 text-xs focus:border-blue-600"
                                                        required
                                                    />
                                                </div>
                                                ...
                                                {/* 진행 장소 */}
                                                <div className="space-y-1">
                                                    <label className="text-[10px] font-bold text-slate-500">위치 (장소)</label>
                                                    <select
                                                        value={mission.location || ''}
                                                        onChange={e => handleUpdateMission(idx, 'location', e.target.value)}
                                                        className="w-full px-3 py-2 bg-white border border-slate-200/80 rounded-lg outline-none font-semibold text-slate-700 text-xs focus:border-blue-600 cursor-pointer"
                                                        required
                                                    >
                                                        <option value="이높플레이스">이높플레이스</option>
                                                        {HYPHEN_DETAILS.map(detail => (
                                                            <option key={detail} value={`하이픈 ${detail}`}>{`하이픈 ${detail}`}</option>
                                                        ))}
                                            ...
                                            {/* 미션 세부 내용 및 인증 가이드 */}
                                            <div className="space-y-1">
                                                <label className="text-[10px] font-bold text-slate-500">수행 방법 (인증 기준)</label>
                                                <textarea
                                                    placeholder="예: 말씀을 통해 하나님과 연결되는 기쁨! 공간에서 말씀 1장 필사하기"
                                                    value={mission.desc || ''}
                                                    onChange={e => handleUpdateMission(idx, 'desc', e.target.value)}
                                                    className="w-full p-3 bg-white border border-slate-200/80 rounded-lg outline-none font-semibold text-slate-700 text-xs focus:border-blue-600 h-16 resize-none"
                                                    required
                                                />
                                            </div>
                                        </div>
