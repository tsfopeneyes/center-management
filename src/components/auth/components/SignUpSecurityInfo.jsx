...
const SignUpSecurityInfo = ({ formData, setFormData, handleChange, agreements, setAgreements, setShowConsentModal, isKiosk }) => {
    return (
        <div className="w-full space-y-4">
            {/* Group 2: Password */}
            <div className={`border ${isKiosk ? 'border-slate-200 rounded-2xl bg-white' : 'border-gray-300 rounded-xl bg-white'} overflow-hidden focus-within:border-[#CF3A27] transition-colors`}>
            ...
            <div className={`border ${isKiosk ? 'border-[#CF3A27]/20 rounded-2xl bg-[#CF3A27]/5' : 'border-[#CF3A27]/20 rounded-xl bg-[#CF3A27]/5'} flex justify-between items-center p-4 transition-colors`}>
                ...
                <div className="flex bg-white rounded-lg border border-[#CF3A27]/20 overflow-hidden shadow-sm">
                    <button
                        type="button" onClick={() => setFormData(prev => ({ ...prev, isSchoolChurch: true }))}
                        className={`px-4 py-2 text-sm font-bold transition-colors ${formData.isSchoolChurch ? 'bg-[#CF3A27] text-white' : 'text-gray-500 hover:bg-gray-50'}`}
                    >
                        네
                    </button>
                    <button
                        type="button" onClick={() => setFormData(prev => ({ ...prev, isSchoolChurch: false }))}
                        className={`px-4 py-2 text-sm font-bold transition-colors border-l border-[#CF3A27]/20 ${!formData.isSchoolChurch ? 'bg-[#CF3A27] text-white' : 'text-[#8B95A1] hover:bg-gray-50'}`}
                    >
                        아니요
                    </button>
                </div>
            ...
            <div className={`space-y-3 ${isKiosk ? 'bg-slate-50 p-6 rounded-[2rem] border border-slate-100' : 'bg-gray-50 p-4 rounded-2xl border border-gray-100'}`}>
                <div className="flex items-start gap-2">
                    <input
                        ...
                        className={`mt-1 w-4 h-4 text-[#CF3A27] rounded focus:ring-[#CF3A27] accent-[#CF3A27] ${isKiosk ? 'w-6 h-6 border-slate-200' : ''}`}
                    />
                    <label htmlFor="total-agree" className={`text-sm leading-snug cursor-pointer ${isKiosk ? 'text-slate-600 font-bold' : 'text-gray-600'}`}>
                        <span className="font-bold text-[#CF3A27]">[전체동의]</span> SCI CENTER 이용 약관 및 개인정보 수집에 동의합니다.
                    </label>
                </div>
                <button
                    type="button" onClick={() => setShowConsentModal(true)}
                    className={`text-xs text-[#CF3A27] font-bold ml-6 underline ${isKiosk ? 'sm:text-sm font-black underline-offset-4' : ''}`}
                >
                    개인정보 수집 및 이용 자세히 보기
                </button>
            </div>
