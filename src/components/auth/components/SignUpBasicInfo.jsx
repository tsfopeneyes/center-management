...
const SignUpBasicInfo = ({ formData, setFormData, handleChange, handlePhoneChange, handleGuardianPhoneChange, isUnder14, isKiosk }) => {
    return (
        <div className="w-full">
            ...
            <div className={`mt-2 border ${isKiosk ? 'border-slate-200 rounded-2xl bg-white' : 'border-gray-300 rounded-xl bg-white'} overflow-hidden focus-within:border-[#CF3A27] transition-colors`}>
                    ...
                    <div className="flex w-1/3 sm:w-1/4">
                        {[
                            ...
                            <button
                                ...
                                className={`flex-1 py-3 text-sm font-bold transition ${formData.gender === g.value ? 'bg-[#CF3A27] text-white' : 'bg-transparent text-gray-500 hover:bg-gray-50'} ${idx === 0 ? 'border-r border-gray-200' : ''} ${isKiosk ? 'sm:py-4 font-black' : ''}`}
                            >
                                {g.label}
                            </button>
                        ))}
                ...
                <div className="flex border-b border-gray-200">
                    {['청소년', '졸업생'].map((g, idx) => (
                        <button
                            ...
                            className={`flex-1 py-3 text-sm font-bold transition ${formData.user_group === g ? 'bg-[#CF3A27] text-white' : 'bg-transparent text-gray-500 hover:bg-gray-50'} ${idx === 0 ? 'border-r border-gray-200' : ''} ${isKiosk ? 'sm:py-4 font-black' : ''}`}
                        >
                            {g}
                        </button>
                    ))}
                ...
                {isUnder14(formData.birth) && (
                    ...
                    >
                        <div className="flex items-center gap-2 mb-2 mt-2">
                            <div className="w-2 h-2 bg-[#CF3A27] rounded-full" />
                            <h4 className={`text-xs font-bold text-[#CF3A27] uppercase tracking-widest ${isKiosk ? 'text-sm' : ''}`}>보호자 정보 (만 14세 미만 필수)</h4>
                        </div>

                        <div className={`border ${isKiosk ? 'border-[#CF3A27]/30 rounded-2xl bg-[#CF3A27]/5' : 'border-[#CF3A27]/30 rounded-xl bg-[#CF3A27]/5'} overflow-hidden focus-within:border-[#CF3A27] transition-colors`}>
                            <div className="flex border-b border-[#CF3A27]/20">
                                <input
                                    type="text" name="guardianName" required
                                    value={formData.guardianName} onChange={handleChange}
                                    placeholder="보호자 이름"
                                    className={`w-1/2 px-4 py-3 bg-transparent border-r border-[#CF3A27]/20 outline-none font-bold text-sm text-[#191F28] placeholder-[#B0B8C1] placeholder:font-bold ${isKiosk ? 'sm:py-4' : ''}`}
                                />
                                <input
